...
import LocationWidget from './widgets/MyCustomWidget'; // Import StopwatchWidget
...
function App() {
  const [widgets, setWidgets] = useState([
    ...
    { id: new Date().getTime() + 3, component: <LocationWidget />, area: 'none-widget', name: "Location" }, // Add LocationWidget
  ]);
      ...
      <div className="none-widget">
        {widgets.length > 0 &&
          widgets.map((widget, index) => {
            ...
            return null; // Added this line to handle the warning for missing return statement
          })}
      </div>
      <div className="App">
        <div className="container">
          <div className="left-widget">
            {widgets.length > 0 &&
              widgets.map((widget, index) => {
                if (widget.area === 'left-widget') {
                  return (
                    <div style={{ padding: 10 }} className="widget-container" key={widget.id}>
                      <div style={{ marginBottom: 10 }}>
                        <select
                          value={widget.area}
                          onChange={(e) => {
            ...
            {widgets.filter(widget => widget.area === 'left-widget').length < 3 &&
              widgets.filter(widget => widget.area === 'left-widget').length > 0 && (
                <button
                  className="add-widget-button"
                  onClick={() => {
                    setShowWidgetModal(true);
          ...
          <div className="right-widget">
            ...
            {widgets.filter(widget => widget.area === 'right-widget').length < 3 &&
              widgets.filter(widget => widget.area === 'right-widget').length > 0 && (
                <button
                  className="add-widget-button"
                  onClick={() => {
                    setShowWidgetModal(true);
